...
def get_opt():
    opt = parse_opts()
    opt.root_data = '.'
    opt.video_path = 'datasets/hmdb51_jpg/'
    opt.annotation_path = 'datasets/hmdb51_1.json'
    opt.result_path = 'results'
    opt.dataset = 'hmdb51'
    opt.model = 'resnet'
    opt.model_depth = 50
    opt.pretrain_path = 'r3d50_KMS_200ep.pth'
    opt.n_pretrain_classes = 1139
    opt.n_classes = 51
    opt.batch_size = 16
    opt.n_threads = 6
    opt.checkpoint = 5
...
def main_worker(index, opt):
    ...
    for i in range(opt.begin_epoch, opt.n_epochs + 1):
        if not opt.no_train:
            ...
            if i % opt.checkpoint == 0 and opt.is_master_node:
                save_file_path = os.path.join(opt.result_path , 'save_{}.pth').format(i)
    ...
    if opt.inference:
        ...
        inference_result_path = os.path.join(opt.result_path , '{}.json').format(
